 ...
 * Date: 2023-12-14
 * Time: 15:07
 * Description: Hashids 协议实现，以实现：生成简短、唯一、大小写敏感并无序的hash值,自然数字的Hash值，可以设置不同的盐，具有保密性，可配置的hash长度，递增的输入产生的输出无法预测
 * $cid = 10;
 * $id = 1555;
 * $hashids = Hashids::instance();
 * $encode_id = $hashids->encode($cid, $id); //加密
 * print_r($encode_id.'===');
...
class Hashids implements HashGenerator {
    ...
    private $_seps = 'dadadezhou';
    private $_min_hash_length = 0;
    private $_math_functions = array();
    private $_max_int_value = 1000000000;

    const hashids_length = 8;           // 加密字符串长度
    const hashids_salt = 'lecms';      // 加密钥匙
    const hashids_alphabet = 'abcdefghijklmnopqrstuvwxyz1234567890';         // 字符仓库,不填写默认为扩展里的字符仓库
     ...
     * @return static
     */
    public static function instance($salt = null, $length = null, $alphabet = null)
    {
    ...
    private function _encode($numbers = array()) {
    ...
    private function _decode($hash, $alphabet) {
        ...
        if( !isset($ret[1]) ){  //加密单个，修改为直接返回单个解密字符串
            return $ret[0];
        }else{
            return $ret;
        }
