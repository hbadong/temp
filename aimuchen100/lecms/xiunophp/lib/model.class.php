...
class model{
	...
	*/
	...
	public function get_field(){
        return $this->db->get_field($this->table);
    }
	 ...
	 */
	public function find_update($where, $data, $order = array(), $limit = 0, $lowprority = FALSE) {
		$this->unique = array();
		...
		return $this->db->find_update($this->table, $where, $data, $order, $limit, $lowprority);
	}

    /**
     * 更新一条数据的浏览量
     * @param array $key	键名数组
     * @param int  $n	浏览量 默认为 + 1
     * @param string $field	浏览量字段
     * @return bool
     */
    public function update_views($key, $n = 1, $field = 'views') {
        $key = $this->arr2key($key);
        return $this->cache_db_update_views($key, $n, $field);
    }
	 ...
	 * @return int	返回影响的记录行数
	 */
	public function find_delete($where, $order = array(), $limit = 0, $lowprority = FALSE) {
		$this->unique = array();
		if($this->cache_conf['enable']) {
			$n = $this->find_count($where);
			if($n > 2000) {
		...
		$num = $this->db->find_delete($this->table, $where, $order, $limit, $lowprority);
     ...
     * @return boot
     */
    public function cache_db_update_views($key, $n = 1, $field = 'views') {
        $this->cache_conf['enable'] && $this->cache->delete($key);
        return $this->db->update_views($key, $n, $field);
    }
