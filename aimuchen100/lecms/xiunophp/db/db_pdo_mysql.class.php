...
class db_pdo_mysql implements db_interface {
	...
	public function __get($var) {
		...
		if($var == 'wlink') {
			...
			empty($cfg['engine']) && $cfg['engine'] = 'InnoDB';
	...
	public function find_fetch_key($table, $pri, $where = array(), $order = array(), $start = 0, $limit = 0) {
		...
		$sql = $s;
		$ret = array();
		//$query = $this->rlink->query($sql);   这个在DEBUG下不记录sql日志
	...
	public function table_create($table, $cols, $engineer = '') {
		empty($engineer) && $engineer = 'InnoDB';
	...
	/**
	 * 插入一条记录（插件模型兼容方法）
	 * @param string $table 完整表名（可带反引号）
	 * @param array $data 列=>值 关联数组
	 * @return int 新记录ID
	 */
	public function insert($table, $data) {
		...
		$stmt = $this->rlink->prepare("INSERT INTO $table ($fields) VALUES ($vals)");
		$stmt->execute(array_values($data));
		return (int)$this->rlink->lastInsertId();
	}
}
