...
class db_mysql implements db_interface {
	...
	public function table_count($table) {
		...
		if($query) {
			...
			throw new Exception('framework_count error, mysql_error:'.mysql_error());
		}
		if($count === FALSE) {
	...
	public function get_field($table){
		$ret = array();
		$cfg = $this->conf['master'];
		$sql = "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE table_name = '".$table."' AND table_schema='".$cfg['name']."'";
